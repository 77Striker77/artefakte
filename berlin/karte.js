...
(function () {
  ...
  var SYMBOL = {
    ...
    // Kalenderblatt mit Stern: ein Termin ist ein DATUM, kein Ort. Muenchen
    // trug hier eine Filmklappe - richtig fuer ein Studio, falsch fuer ein
    // Planetarium, und die beiden Termine dieser Reise sind verschieden genug,
    // dass ein gegenstaendliches Zeichen bei einem von beiden luegen wuerde.
    termin: '<rect x="3" y="5" width="18" height="16" rx="2"/><path d="M3 10h18"/>'
          + '<path d="M8 3v4M16 3v4"/>'
          + '<path d="m12 13 1.2 2.5 2.8.4-2 2 .5 2.7-2.5-1.3-2.5 1.3.5-2.7-2-2 2.8-.4z"/>',
    // Currywurst: Pappschale mit Pommesgabel. Das Berliner Gegenstueck zum
    // Masskrug, den Muenchen hier trug.
    imbiss: '<path d="M4 12h16l-1.5 8a2 2 0 0 1-2 1.6H7.5a2 2 0 0 1-2-1.6z"/>'
          + '<path d="M7 12V6M10 12V4M13 12V6"/><path d="M17 12V8a2 2 0 0 1 4 0"/>',
    // Saeule mit Kapitell - steht fuer Tore, Denkmaeler und historische Bauten
    // gleichermassen; ein Kirchturm haette bei Reichstag und Brandenburger Tor
    // gepasst, bei Gedenkstaetten aber nicht.
  ...
  // KEINE neue Farbe (siehe stil.css).
  //
  // MUENCHEN TRENNTE NACH KUECHE, BERLIN NACH ZEITFENSTER. Dort waren
  // "Fruehstueck / Abendessen / Smash / Burger / Fast Food" sinnvoll, weil die
  // Tage offen waren und die Frage "worauf habe ich Lust" lautete. Diese Reise
  // hat zwei feste Termine, und die Frage lautet "was hat offen, wenn ich
  // Zeit habe". Eine Kategorie, die das nicht beantwortet, ist hier Zierde.
  //
  // bau-oeffentlich.mjs bricht ab, wenn ein Ort eine unbekannte Kategorie
  // traegt: er laege auf keiner Filterebene, und die Karte saehe trotzdem
  // vollstaendig aus. Genau dieser Fehler hat in Muenchen den Innenstadt-Reiter
  // gekostet.
  var GASTRO = [
    { id: "klassiker",   label: "Berliner Klassiker", symbol: "imbiss" },
    { id: "fruehstueck", label: "Frühstück",          symbol: "cafe" },
    { id: "luecke",      label: "Zwischen den Shows", symbol: "essen" },
    { id: "spaet",       label: "Spät am Freitag",    symbol: "burger" }
  ];
  ...
  var quellen = [
    {
      ...
      was: "Ankunftspunkt, Unterkunft, Zentrum, Parks und die beiden Termine. Jeder Punkt "
  ...
  if (DATEN.wahrzeichen_quelle) {
    ...
    quellen.push({
      ...
      was: "Die AUSWAHL ist kuratiert, nicht abgefragt: eine Abfrage auf Sehenswürdigkeiten "
         + "liefert in Berlin vierstellig viele Treffer, vom Brandenburger Tor bis zur "
         + "Gedenktafel im Hinterhof. Was auf eine Reisekarte gehört, ist eine Entscheidung. "
         + "Geholt wird nur die Koordinate — und der angezeigte Name kommt aus der Antwort, "
         + "nicht aus der Suche: so fällt auf, wenn etwas anderes gefunden wurde als gemeint "
         + "war. Beim Aufbau ist das sofort eingetreten — „Asisi Panorama Berlin“ trifft ein "
         + "<em>anderes</em> Panorama zwei Kilometer weiter (Die Mauer, Friedrichstraße); das "
         + "Pergamon-Panorama hängt jetzt an seiner Straße statt an seinem Markennamen."
         + (wzz.filter(function (o) { return o.zu; }).length
             ? " <strong>Geschlossene Häuser bleiben auf der Karte</strong>, mit Grund und "
               + "Quelle statt stillschweigend gelöscht: das Pergamonmuseum ist in "
               + "Grundinstandsetzung, und wer es hier vermisst, recherchiert beim nächsten "
               + "Mal dasselbe Haus erneut."
             : "")
    });
  }
  ...
  if (DATEN.gastro) {
    ...
    var gz = function (id, pruef) {
      return DATEN.gastro.orte.filter(function (o) { return o.kategorie === id && pruef(o); }).length;
    };
    ...
    var GASTRO_QUELLTEXT = {
      klassiker: function (n) {
        return "Die AUSWAHL ist recherchiert, nicht abgefragt: Berliner Institutionen, die es "
          + "seit Jahrzehnten gibt — die älteste seit 1621. Koordinate, Adresse, Note, Stimmen "
          + "und Öffnungszeiten kommen aus der Places API und damit vom Betrieb selbst. "
          + "<strong>Diese Gruppe ist an kein Zeitfenster gebunden</strong> und trägt darum als "
          + "einzige keine Fensterprüfung: sie gehört zur Stadt, nicht zu dieser Reise, und "
          + "überlebt sie.";
      },
      fruehstueck: function (n) {
        return "Kuratiert im Abrufskript, Werte aus der Places API. Geprüft ist zusätzlich, ob "
          + "der Laden <strong>am Freitag zwischen 09:00 und 12:00</strong> offen hat — das ist "
          + "nicht Zierde: an diesem Tag gibt es zwischen 13:30 und etwa 20:30 planbar nichts zu "
          + "essen, weil das Studio keine Pause hat. "
          + wort(gz("fruehstueck", keineZeit)).replace(/^./, function (c) { return c.toUpperCase(); })
          + (gz("fruehstueck", keineZeit) === 1 ? " Öffnungszeit liegt" : " Öffnungszeiten liegen")
          + " nicht vor und steht als <em>unbekannt</em> — nicht als „offen“.";
      },
      luecke: function (n) {
        return "<strong>Die einzige Gruppe, die eine Ortsbedingung trägt.</strong> Alle Einträge "
          + "sind zu Fuß vom Zeiss-Großplanetarium erreichbar; die Luftlinie steht je Ort dabei "
          + "und ist <em>Luftlinie</em>, keine Gehzeit — gemessene Gehzeiten liefert die "
          + "Umgebungskarte auf demselben Reiter. "
          + "Geprüft ist, ob der Laden <strong>am Samstag zwischen 17:45 und 19:45</strong> offen "
          + "hat, also in der Lücke zwischen den beiden Vorstellungen. "
          + "Die Prüfung kennt fünf Ergebnisse und wirft sie nicht zusammen: <em>offen</em>, "
          + "<em>öffnet spät</em> (macht in der Lücke auf und bleibt dann offen), <em>knapp</em> "
          + "(schließt in der Lücke), <em>teilweise</em> und <em>zu</em>. "
          + "Der erste Entwurf kannte nur offen/zu und hat damit sofort einen Fehlbefund erzeugt — "
          + "ein Lokal mit 4,7 ★, das 15 Minuten nach Beginn der Lücke aufmacht, stand als "
          + "„geschlossen“ da. Ein falsches Nein sieht genauso sauber aus wie ein richtiges.";
      },
      spaet: function (n) {
        return "Geprüft ist, ob der Laden <strong>am Freitag um 21:45</strong> noch offen hat — "
          + "die Aufzeichnung endet laut Veranstalter gegen 20:30, der Rückweg dauert gemessene "
          + "45 Minuten. Ein Lokal, das um 21:00 schließt, ist für diesen Abend kein Lokal, auch "
          + "wenn es tagsüber das beste wäre. "
          + wort(gz("spaet", keineZeit)).replace(/^./, function (c) { return c.toUpperCase(); })
          + (gz("spaet", keineZeit) === 1 ? " Eintrag führt" : " Einträge führen")
          + " bei Google keine Zeiten und steht darum als <em>unbekannt</em>: das ist ein Ort, bei "
          + "dem man anruft, kein Ort, der offen hat.";
      }
    };
    GASTRO.forEach(function (k) {
      var qq = DATEN.gastro.quelle;
      var n = (DATEN.gastro.anzahl.je_kategorie || {})[k.id] || 0;
      // Ohne Text kein Kaertchen. Diese Zeile fehlte in Muenchen beim
      // Nachtragen einer Kategorie und hat die ganze Seite angehalten: eine
      // Kategorie ohne Quellentext ist ein Aufruf auf undefined.
      if (!qq || !n || !GASTRO_QUELLTEXT[k.id]) return;
      var f = (DATEN.gastro.fenster || {})[k.id];
      quellen.push({
        titel: k.label,
        menge: n + " Orte" + (f ? " · Fenster " + f.was : ""),
        quelle: qq.name,
  ...
  hinweis.innerHTML = "<strong>Was hier nicht steht.</strong> Diese Seite zeigt Reisedaten, "
    + "Unterkunft, Züge und Gleise — alles Angaben, die ohnehin auf jeder Abfahrtstafel, in "
    + "jedem Stadtplan oder auf der Hotelseite stehen. <em>Nicht</em> hier stehen "
    + "Buchungs-, Auftrags-, Ticket-, Vorgangs- und Reservierungsnummern, Stornocodes, Preise, "
    + "Namen und Zahlungsdaten; ein Tor im Veröffentlichungsskript prüft darauf und bricht ab, "
    + "wenn eines davon auftaucht. Unbekanntes steht als <em>xx</em> und wird nicht geschätzt.";
  ...
  // --- Die Termine: der ANLASS, nicht der Ort ------------------------------
  // Muenchens Fassung dieses Blocks traegt die teuerste Lektion des Cockpits:
  // der Reiter hiess dort zuerst "Bavaria Filmstadt" und zeigte Eintrittspreise
  // und Kulissen der Besucherfuehrung - gefahren wurde aber zu einer
  // Fernsehaufzeichnung. Ein Reiter, der aus dem ORTSNAMEN gebaut wird statt
  // aus dem ANLASS, sieht vollstaendig aus und beantwortet die falsche Frage.
  //
  // Darum heisst die Datenstruktur hier 'termine' und nicht 'orte': der Ort ist
  // ein Feld des Termins, nicht umgekehrt.
  //
  // Gebaut nach dem Tagesablauf, nicht nach der Datenherkunft: oben was wann
  // ist, darunter was mitmuss, dann die Regeln des Hauses, zuletzt die
  // Anbindung. Was in der Quelle nicht steht, steht auch hier nicht.
  //
  // EINE SCHLEIFE STATT ZWEIER BLOECKE. Berlin hat zwei Termine, und der zweite
  // ist kein zweiter Block: er ist ein Durchlauf. Ein dritter waere ein Eintrag
  // in reise.json und keine Aenderung hier.
  //
  // Was NICHT hier steht, fehlt aus einem Grund: Ticketnummer, Vorgangsnummer,
  // Stornocode und Name sind personenbezogen. Das Datenschutz-Tor in
  // deploy-pages.sh prueft darauf und bricht ab.
  (DATEN.termine || []).forEach(function (e) {
    var el = document.getElementById("termin-" + e.id + "-tafel");
    if (!el) return;
    var be = e.eintrag;
    var bw = e.weg;
    ...
    var quellen = [e.quelle,
                   ...
                   e.umgebung && e.umgebung.quellen
                     ? e.umgebung.quellen.halte.name + " (Haltestellen und Linien)"
    ...
    el.innerHTML = '<div class="fahrt">'
      ...
      + '<details class="quell-klapp"><summary>Woher diese Angaben kommen</summary>'
      + "<p>" + quellen
      + " · Ticket-, Vorgangs- und Stornonummern sowie Namen stehen nicht auf dieser Seite.</p>"
      + "</details>"
      + "</div>";
  });
  ...
  // Dieselbe Bauart, drei Orte. Der Hotel-Reiter beantwortet "wo liegt die
  // Unterkunft und wie komme ich vom Bahnhof hin", die Termin-Reiter "wo liegt
  // der Termin und wie komme ich vom Hotel hin". Dieselbe Frage, ein anderer
  // Bezugspunkt - und darum dieselbe Funktion und keine zweite Fassung.
  //
  // Der Startpunkt des Wegs ist das, was auf dem jeweiligen Reiter schon
  // feststeht: fuer das Hotel der Bahnhof, fuer einen Termin das Hotel.
  wegKarte({
    id: "hotel",
  ...
  // Je Termin eine eigene Karte mit eigenem Untermenue. Die Behaelter-Kennungen
  // werden aus der Termin-id GERECHNET, nicht gelistet: eine Liste und eine
  // Schleife koennen auseinanderlaufen, eine Rechnung nicht. Fehlt der
  // Behaelter im HTML, steigt wegKarte() von selbst aus.
  (DATEN.termine || []).forEach(function (t) {
    wegKarte({
      id: t.id,
      umgebung: t.umgebung,
      weg: t.weg,
      karte: "karte-" + t.id,
      reiter: t.id + "-reiter",
      tafel: t.id + "-weg-tafel",
      legende: "legende-" + t.id,
      fuss: t.id + "-fuss",
      ziel_art: "termin",
  ...
  (function () {
    var g = DATEN.gastro;
    if (!g || !g.orte.length || !document.getElementById("karte-mitte")) return;
    ...
    // Die Gruppe IST die Kategorie. Muenchen brauchte hier eine Fallunter-
    // scheidung, weil die Burger zusaetzlich nach `smash.art` eingefaerbt
    // wurden; Berlin trennt nach Zeitfenster, und ein Zeitfenster hat keine
    // Unterarten. Die Funktion bleibt trotzdem stehen statt wegzufallen: sie
    // ist die eine Stelle, an der sich das aendern liesse.
    var gruppeVon = function (o) { return o.kategorie; };
    ...
    var REITER = [
      ...
      // Die Knoepfe sind die ZEITFENSTER dieser Reise, nicht die Kuechen.
      // "Zwischen den Shows" ist keine Kueche - es ist die einzige Luecke des
      // Samstags, und wer sie sucht, sucht sie unter diesem Namen.
      { id: "klassiker",   label: "Klassiker",   gruppen: ["klassiker"] },
      { id: "fruehstueck", label: "Frühstück",   gruppen: ["fruehstueck"] },
      { id: "luecke",      label: "Zwischen den Shows", gruppen: ["luecke"] },
      { id: "spaet",       label: "Spät",        gruppen: ["spaet"] },
      { id: "sehenswert",  label: "Sehenswertes", gruppen: ["wahrzeichen", "museum"] }
    ];
    var reiterAktiv = "alle";

    // ===== Karte ============================================================
    karteInnen = L.map("karte-mitte", { scrollWheelZoom: true })
    ...
    alle.forEach(function (e) {
      ...
      // Das FENSTER-Ergebnis steht ganz oben, vor der Adresse und vor allem
      // anderen: bei dieser Reise ist "hat das offen, wenn ich Zeit habe" die
      // erste Frage und nicht die letzte.
      var FENSTER_TEXT = {
        "offen":         "offen im ganzen Fenster",
        "oeffnet-spaet": "macht im Fenster auf und bleibt offen",
        "knapp":         "schließt im Fenster",
        "teilweise":     "nur ein Teil des Fensters",
        "zu":            "an dem Tag zu",
        "unbekannt":     "unbekannt — keine Zeiten hinterlegt"
      };
      var fakten = (o.fenster
          ? faktenZeile(o.fenster.was, FENSTER_TEXT[o.fenster.ergebnis]
              + (o.fenster.oeffnet ? " (ab " + o.fenster.oeffnet + ")" : "")
              + (o.fenster.schliesst ? " (bis " + o.fenster.schliesst + ")" : "")
              + (o.fenster.beleg ? '<span class="ort-beleg">' + o.fenster.beleg + "</span>" : ""))
          : "")
        + faktenZeile("Offen", e.w ? zeitenKurz(e.w) : "unbekannt — nicht erhoben")
        + (o.luftlinie_m ? faktenZeile("Luftlinie", o.luftlinie_m + " m — keine Gehzeit") : "")
      ...
      d.querySelector(".ort-zeigen").addEventListener("click", function () {
        ...
        e.marke.openPopup();
        document.getElementById("karte-mitte").scrollIntoView({ block: "center" });
      });
    ...
    var umschalten = function (soll, ordnen) {
      ...
      if (ruhig || !document.getElementById("feld-mitte").offsetParent) {
    ...
    var navEl = document.getElementById("mitte-reiter");
    var knoepfeI = [];
    ...
    var ulI = document.getElementById("legende-mitte");
    ...
    var gz = function (id, pruef) {
      return g.orte.filter(function (o) { return o.kategorie === id && pruef(o); }).length;
    };
    ...
    var q = g.quelle;
    var fz = function (e) { return g.orte.filter(function (o) { return o.fenster && o.fenster.ergebnis === e; }).length; };
    ...
    document.getElementById("gastro-fuss").innerHTML =
      '<span class="fuss-zahlen">'
        + GASTRO.map(function (k) {
            var n = (g.anzahl.je_kategorie || {})[k.id] || 0;
            return '<b>' + n + "</b> " + k.label;
          }).join("")
        + '<b>' + fz("offen") + "</b> im Fenster offen"
        + '<b>' + (fz("knapp") + fz("oeffnet-spaet") + fz("teilweise")) + "</b> nur teilweise"
        + '<b>' + fz("zu") + "</b> im Fenster zu"
      ...
      + "<p>Ein Bestand, vier Gruppen, Rang <em>" + q.rang + "</em>, am "
        + deutsch(q.abgerufen) + " abgerufen: Koordinate, Adresse, Bewertung und "
      + "<strong>Öffnungszeiten</strong> kommen aus der <strong>Google Places API</strong>, "
      + "die Auswahl und alle Beschreibungen aus dem Abrufskript. Kein Preis ist von einer "
      + "Karte vor Ort abgelesen.</p>"
      + "<p><strong>Die Öffnungszeiten werden geprüft, nicht nur angezeigt.</strong> Je Gruppe "
      + "gibt es ein Zeitfenster dieser Reise — Freitagvormittag, die Lücke zwischen den beiden "
      + "Vorstellungen am Samstag, Freitagnacht nach der Aufzeichnung —, und jeder Ort trägt das "
      + "Ergebnis dieser Prüfung samt der Zeile, aus der es gerechnet wurde. Die "
      + "<em>Klassiker</em> tragen keines: sie gehören zur Stadt, nicht zu dieser Reise.</p>"
      + "<p><strong>Der angezeigte Name kommt aus der Antwort, nicht aus der Frage.</strong> "
      + "Diese Probe hat beim Aufbau einen echten Fehlgriff gefunden: die Suche nach „Mio Matto, "
      + "Karl-Liebknecht-Straße“ lieferte „Mio Berlin“ in der Panoramastraße — ein anderer "
      + "Betrieb an einer anderen Adresse, der mit echter Bewertung auf der Karte gestanden "
      + "hätte. Der Eintrag ist daraufhin ersetzt worden, mit Begründung im Skript. Zurzeit "
      + "weichen <strong>" + abweichend + "</strong> Namen ab; wo einer abweicht, steht es beim "
      + "Ort — meist ist es nur eine andere Schreibweise desselben Hauses.</p>"
      + "<p><strong>Was fehlt:</strong> " + ohneZeit + " Ort(e) führen bei Google keine Zeiten. "
      + "Sie stehen als <em>unbekannt</em> und nicht als <em>offen</em> — das ist ein Ort, bei "
      + "dem man anruft. Die Luftlinie je Ort ist <em>Luftlinie</em>, keine Gehzeit; gemessene "
      + "Gehzeiten liefert die Umgebungskarte auf dem Hotel-Reiter.</p>"
      + "</details>"
  ...
  function zeige(id) {
    ...
    if (id === "mitte" && karteInnen) karteInnen.invalidateSize();
