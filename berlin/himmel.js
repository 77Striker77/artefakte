...
(function () {
  ...
  // GERECHNET am 19.09.2026, nicht geschaetzt: bei 0,22 Deckung steht der
  // schwaechste Textton der Seite (--auf-grund-leise, #C0B3E0) auf dem
  // rechnerisch hellsten Stern (--auf-grund #F0ECFB voll deckend, ueber diesen
  // Deckel gelegt: Grund #453F5B) noch bei 5,09:1. Bei 0,26 faellt er auf
  // 4,43:1 und reisst.
  //
  // RECHNERISCH und nicht gemessen, weil die Messung zufaellig ist: welcher
  // Stern gerade der hellste ist, entscheidet Math.random(). Zwei Laeufe
  // ergaben 0,196 und 0,204 - ein Nachweis, der so schwankt, ist keiner.
  // pruef-farben.mjs misst den Fall trotzdem mit; es ist die Gegenprobe zur
  // Rechnung, nicht ihr Ersatz.
  var DECKEL = 0.22;
  ...
  var PALETTE = [
    // Die Rueckfallwerte sind die BERLINER Werte aus stil.css. Sie greifen nur,
    // wenn die CSS-Variable fehlt - dann aber sichtbar, und ein uebernommener
    // Muenchner Ziegelton faerbte den Himmel stillschweigend rot.
    farbe("--auf-grund", "#F0ECFB"),
    farbe("--auf-grund-leise", "#C0B3E0"),
    farbe("--auf-grund-leise", "#C0B3E0"),
    farbe("--m-termin", "#7C52E0"),
    farbe("--m-unterkunft", "#08788F")
  ];
